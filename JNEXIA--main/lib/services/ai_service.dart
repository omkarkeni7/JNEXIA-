...
import 'backend_ai_service.dart' as backend;
...
class AIService {
  ...
  /// Tries backend first (for better context and history), then falls back to direct API.
  static Future<String?> getAIResponse(String userInput, String language, {String? studentContext}) async {
    // Try backend first (if available)
    try {
      final backendResponse = await backend.BackendAIService.getAIResponse(
        userInput,
        language,
        studentContext: studentContext,
      );
      if (backendResponse != null) {
        print('[AIService] Using backend AI (with conversation history)');
        return backendResponse;
      }
    } catch (e) {
      print('[AIService] Backend not available, using direct API: $e');
    }

    // Fallback to direct Mistral API
    try {
